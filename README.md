...
docs/                         <- served by GitHub Pages
  ...
  sw.js                       <- service worker (cache name: timesheet-v6)
...
When you change `index.html` or `sw.js`, bump the cache name in `sw.js` (`timesheet-v6` → `v7`, etc.) so installed PWAs pick up the new shell.
